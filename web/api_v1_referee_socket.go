...
type apiV1RefereeSocketCommand struct {
...
type apiV1RefereeSocketResult struct {
	RequestId string `json:"requestId"`
...
func (web *Web) handleRefereeV1Commands(ws *websocket.Websocket, r *http.Request) {
	defer ws.Close()
	...
	for {
		...
		result := apiV1RefereeSocketResult{}
		if messageType != "command" {
			...
			if marshalErr != nil || json.Unmarshal(body, &input) != nil {
				...
				default:
					commandResult, commandErr := web.executeRefereeIdempotent(input.IdempotencyKey, input.Command, input.Payload)
					result.Changed, result.FoulId, result.Replayed = commandResult.Changed, commandResult.FoulId, commandResult.Replayed
					switch {
					case errors.Is(commandErr, errAllianceSelectionKeyReused):
						result.Code = "idempotency_key_reused"
					case errors.Is(commandErr, errInvalidRefereeCommand):
						result.Code = "unknown_command"
					case errors.Is(commandErr, errRefereeScoreCommitted):
				...
				web.logApiV1Audit(r, "referee."+input.Command, "score", outcome)
			}
		}
